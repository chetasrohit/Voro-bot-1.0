...
function setupAdminRoutes(app) {
  ...
  app.get('/api/admin/topics/:id', (req, res) => {
    ...
    db.get('SELECT * FROM topics WHERE id = ?', [topicId], (err, topic) => {
      ...
      db.all('SELECT * FROM options WHERE topic_id = ? ORDER BY option_key', [topicId], (err, options) => {
        if (err) {
          res.status(500).json({ error: err.message });
          db.close();
          return;
  ...
  app.post('/api/admin/topics/:topicId/options', (req, res) => {
    ...
    if (!option_key || !title || !answer) {
      res.status(400).json({ error: 'option_key, title, and answer are required' });
      return;
    }
  ...
  app.put('/api/admin/feedback/:id', (req, res) => {
    ...
    if (!status || !['new', 'read', 'replied', 'archived'].includes(status)) {
      res.status(400).json({ error: 'Valid status is required (new, read, replied, archived)' });
      return;
    }
